...
do
        ...
        sudo timeout 0.1 sudo tcpdump -i enp0s31f6
...
if [[ $? -eq 0 ]]; then
        ...
        case "$choice" in
        "0")
                exec /usr/bin/steam -tcp -offline -silent -no-browser -applaunch 4000 +connect cinema.swampservers.net:27015 -windowed -safe -noaddons -nochromium -console >/dev/null 2>/dev/null &
                ;;
        "1")
